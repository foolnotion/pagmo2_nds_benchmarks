...
BOOST_AUTO_TEST_CASE(deductive_sorting_test)
{
    auto print = [&](auto& fr) {
    ...
    auto retval = deductive_sorting(example);
    std::cout << "returned:\n";
    print(std::get<0>(retval));
    std::cout << "expected:\n";
    print(non_dom_fronts_res);

    std::cout << "rank expected:\n";
    for (auto v : non_dom_rank_res) { std::cout << v << " "; }
    std::cout << "\n";
    std::cout << "rank returned:\n";
    for (auto v : std::get<3>(retval)) { std::cout << v << " "; }
    std::cout << "\n";
    ...
    retval = deductive_sorting(example);
    BOOST_CHECK(std::get<0>(retval) == non_dom_fronts_res);
    //BOOST_CHECK(std::get<1>(retval) == dom_list_res);
    //BOOST_CHECK(std::get<2>(retval) == dom_count_res);
    BOOST_CHECK(std::get<3>(retval) == non_dom_rank_res);
    ...
    // Test 4
    example = {{0, 0, 0}};
    BOOST_CHECK_THROW(deductive_sorting(example), std::invalid_argument);
    example = {{}};
    BOOST_CHECK_THROW(deductive_sorting(example), std::invalid_argument);
    example = {};
    BOOST_CHECK_THROW(deductive_sorting(example), std::invalid_argument);
...
BOOST_AUTO_TEST_CASE(efficient_sorting_binary_test)
{
    auto print = [&](auto& fr) {
        for (size_t i = 0; i < fr.size(); ++i) {
            std::cout << i << ": ";
    ...
    auto retval = efficient_sorting_binary(example);
    ...
    retval = efficient_sorting_binary(example);
    std::cout << "returned:\n";
    print(std::get<0>(retval));
    std::cout << "expected:\n";
    print(non_dom_fronts_res);
    ...
    // Test 4
    example = {{0, 0, 0}};
    BOOST_CHECK_THROW(efficient_sorting_binary(example), std::invalid_argument);
    example = {{}};
    BOOST_CHECK_THROW(efficient_sorting_binary(example), std::invalid_argument);
    example = {};
    BOOST_CHECK_THROW(efficient_sorting_binary(example), std::invalid_argument);

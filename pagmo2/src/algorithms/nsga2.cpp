...
nsga2::nsga2(unsigned gen, double cr, double eta_c, double m, double eta_m, unsigned seed,
             non_dominated_sorting_algorithm nds_algorithm, bool dominate_on_equal)
    : m_gen(gen), m_cr(cr), m_eta_c(eta_c), m_m(m), m_eta_m(eta_m), m_e(seed), m_seed(seed),
      m_nds_algorithm(nds_algorithm), m_dominate_on_equal(dominate_on_equal), m_verbosity(0u)
{
    if (cr >= 1. || cr < 0.) {
...
population nsga2::evolve(population pop) const
{
    ...
    for (decltype(m_gen) gen = 1u; gen <= m_gen; gen++) {
        ...
        //std::cout << "algorithm: " << (size_t)m_nds_algorithm << "\n";
        auto fnds_res = non_dominated_sorting(pop.get_f(), m_nds_algorithm, m_dominate_on_equal);
        ...
        // operator
        best_idx = select_best_N_mo(popnew.get_f(), NP, m_nds_algorithm, m_dominate_on_equal);

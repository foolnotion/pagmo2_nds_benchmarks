 ...
 */
std::vector<pop_size_t> select_best_N_mo(const std::vector<vector_double> &input_f, pop_size_t N, non_dominated_sorting_algorithm alg, bool dominate_on_equal)
{
    if (N == 0u) { // corner case
        return {};
    }
    ...
    auto tuple = non_dominated_sorting(input_f, alg, dominate_on_equal);
